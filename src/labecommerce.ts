...
app.get("/purchases", async (req: Request, res: Response) => {
    const q = req.query.q as string | undefined
            ...
            try{
                ...
                if(q === undefined){
                    res.status(200).send({message: "nome do cliente deve ser informado em busca"})
                }else{

               const idBuyer = await db.raw(`SELECT id FROM users WHERE name LIKE "%${q}%"`)     
              const buyerDB = await db.raw(`
                    SELECT
                    purchases.id,
                    purchases.quantity,
                    purchases.total_price,
                    purchases.buyer_id
                    FROM purchases
                    INNER JOIN purchases ON buyer_id=${['buyerDB']}`)

                 if(buyerDB){
                    res.send({message: `lista de pagamentos de ${q}` , buyerDB })
                     }  else{
                        res.send(`Nenhum pagamento registrado a nome ${q}`)
                     } 
                }
                } catch (error) {
                console.log(error)
